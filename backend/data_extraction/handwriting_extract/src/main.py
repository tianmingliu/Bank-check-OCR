...
from backend.data_extraction.handwriting_extract.src.SamplePreprocessor import preprocessor, wer
from backend.data_extraction.handwriting_extract.src.Model import DecoderType, Model
from backend.data_extraction.handwriting_extract.src.SpellChecker import correct_sentence
...
def load_different_image():
    ...
    for i in range(1, Model.batchSize):
       filename = os.path.join(filepath, "../data/check_image/a ({}).png".format(i))
       imgs.append(preprocessor(cv2.imread(filename, cv2.IMREAD_GRAYSCALE), Model.imgSize, enhance=False))
    return imgs
...
#     parser.add_argument(
#         "--bestpath", help="use best path decoding instead of word beam search", action="store_true")
...
def extract(image):
    ...
    filename = os.path.join(filedir, 'temp.png')
