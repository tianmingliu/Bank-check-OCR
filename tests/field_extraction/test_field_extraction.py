...
- Extract text from an image
    - For each field:
        ...
        - With isolated text
- Automate extraction tests some. Provide a struct with preprocessing info,
  field extraction info (line detection, text isolation)


"""

file_base_dir      = "tests/test-files/field_extract/"
...
# tests can take advantage of
"""
BEGIN TEST HELPERS

The following tests allow for clearer output for a user.
...
\tExpected [str(expected)]
    ...
    - Used for a specific test. Lets a user know the scenario being tests
...
Preprocess files are written to tests/test-files/field_extract/processed
"""
def setup():
...
def test_bounding_box_merge():
    ...
    list = [box_a, box_b]
    act = fe.merge_overlapping_bb(None, list)
...
def test_bounding_box_merge_close():
    ...
    act = fe.merge_close_bb(None, [box_a, box_b], x_threshold, y_threshold)
...
def test_split_image():
    ...
    test_header("Crop the top part of the image with varying sizes. Output name is tests/test-files/field_extrac/output/crop_image/img_name_upper_crop.jpg")
    ...
    for file in filenames:
        ...
        for split in upper_split:
            ...
            new_image, old_image = fe.crop(new_image, min_x, min_y, max_x, max_y)
    ...
    test_header("Crop the middle part of the image with varying sizes. Output name is tests/test-files/field_extrac/output/crop_image/img_name_middle_crop.jpg")
    ...
    for file in filenames:
        ...
        pay_row_image, old_image = fe.crop(cropped_image, min_x, min_y, max_x, max_y)
        ...
        while current_min_percent < start_max_percent:
            ...
            while current_max_percent > current_min_percent:
                ...
                # crop + write image to file
                new_image, old_image = fe.crop(pay_row_image, min_x, min_y, max_x, max_y)
                write_image(out_filename, new_image)
        ...
        pay_col_image, old_image = fe.crop(cropped_image, min_x, min_y, max_x, max_y)
        ...
        # Use the previous test results to determine the bounds rather than writing duplicate test code.
        ...
        written_image, old_image = fe.crop(cropped_image, min_x, min_y, max_x, cropped_height)
        ...
        while current_min_percent < start_max_percent:
            ...
            while current_max_percent > current_min_percent:
                ...
                # crop + write image to file
                new_image, old_image = fe.crop(written_image, min_x, min_y, max_x, max_y)
                write_image(out_filename, new_image)
